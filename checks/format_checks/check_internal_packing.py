...
def check_internal_packing(
    ...
    if not _PYFIVE_OK:
        ...
        ctx.add_failure(
            f"Optional dependency 'pyfive >= 1.1.2' is not installed or "
    ...
    try:
        f = _acquire_internal_packing_file(ds, file_path)
    except Exception as e:
        ctx = TestCtx(default_severity, "[FILE004] Internal packing")
    ...
    # ----------------------------------------------------------------
    if run_metadata:
        ctx_a = TestCtx(sev_metadata, "[FILE004a] Internal packing : Consolidated internal metadata")
        try:
            if f.consolidated_metadata:
                ctx_a.add_pass()
            else:
                qualifier = severity_word(sev_metadata)
                ctx_a.add_failure(
    ...
    if run_time or run_data:
        try:
            ...
            has_time = False
            if run_time:
                ctx_b = TestCtx(sev_time, "[FILE004b] Internal packing : Time coordinate chunking")
    ...
    if run_time and has_time:
        ...
        if t is not None:
            ...
            if ok:
                ...
                qualifier = severity_word(sev_time)
                ctx_b.add_failure(
    ...
    if run_data:
        try:
            ...
            ctx_d = TestCtx(sev_data, "[FILE004d] Internal packing : Data variable chunking")
        ...
        if has_variable_id_attr:
            ...
            if has_variable and d is not None:
                ...
                if ok:
                    ...
                    qualifier = severity_word(sev_data)
                    ctx_d.add_failure(
